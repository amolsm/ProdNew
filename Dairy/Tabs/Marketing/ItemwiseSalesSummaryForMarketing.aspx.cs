...
namespace Dairy.Tabs.Marketing
{
    public partial class ItemwiseSalesSummaryForMarketing : System.Web.UI.Page
    {
        ...
        protected void btngenrateBill_click(object sender, EventArgs e)
        {
            ...
            DS = billdata.MarketingItemWiseSalesSummarybyDate((Convert.ToDateTime(txtStartDate.Text)).ToString("dd-MM-yyyy"), (Convert.ToDateTime(txtEndDate.Text)).ToString("dd-MM-yyyy"), Convert.ToInt32(dpAgent.SelectedItem.Value));
            if (!Comman.Comman.IsDataSetEmpty(DS))
            {
