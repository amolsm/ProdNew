...
{
    public partial class ReturnComparisonReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                DS = BindCommanData.BindCommanDropDwon("AgentID", "AgentCode+' '+AgentName as Name", "AgentMaster", "IsArchive=0  Order by AgentCode");
                if (!Comman.Comman.IsDataSetEmpty(DS))
                {
        ...
        protected void btngenrateBill_click(object sender, EventArgs e)
        {
            ...
            DS = billdata.ReturnComparisionreportbyDate((Convert.ToDateTime(txtStart1Date.Text)).ToString("dd-MM-yyyy"), (Convert.ToDateTime(txtEnd1Date.Text)).ToString("dd-MM-yyyy"), (Convert.ToDateTime(txtStart2Date.Text)).ToString("dd-MM-yyyy"), (Convert.ToDateTime(txtEnd2Date.Text)).ToString("dd-MM-yyyy"),Convert.ToInt32(dpRoute.SelectedItem.Value), Convert.ToInt32(dpAgent.SelectedItem.Value), Convert.ToInt32(dpBrand.SelectedValue));
            if (!Comman.Comman.IsDataSetEmpty(DS))
            {
